...
import timechat.fcgz.sj.time.model.GroupMemberProfile;
...
public class GroupMemberProfileActivity extends FragmentActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(timechat.fcgz.sj.time.R.layout.activity_group_member_profile);
        ...
        quietingOpt = new String[] {getString(timechat.fcgz.sj.time.R.string.group_member_quiet_cancel)};
        quietOpt = new String[] {getString(timechat.fcgz.sj.time.R.string.group_member_quiet_ten_min),
                getString(timechat.fcgz.sj.time.R.string.group_member_quiet_one_hour),
                getString(timechat.fcgz.sj.time.R.string.group_member_quiet_one_day),
        };
        TemplateTitle title = (TemplateTitle) findViewById(timechat.fcgz.sj.time.R.id.GroupMemTitle);
        ...
        TextView tvKick = (TextView) findViewById(timechat.fcgz.sj.time.R.id.kick);
            ...
            public void onClick(View v) {
                TIMGroupManager.getInstance().deleteGroupMember(groupIdentify, Collections.singletonList(userIdentify), new TIMValueCallBack<List<TIMGroupMemberResult>>() {
                    ...
                    public void onError(int i, String s) {
                        Toast.makeText(GroupMemberProfileActivity.this, getString(timechat.fcgz.sj.time.R.string.group_member_del_err), Toast.LENGTH_SHORT).show();
                    }

                    @Override
                    public void onSuccess(List<TIMGroupMemberResult> timGroupMemberResults) {
                        Toast.makeText(GroupMemberProfileActivity.this, getString(timechat.fcgz.sj.time.R.string.group_member_del_succ), Toast.LENGTH_SHORT).show();
                        setBackResult(true);
                        finish();
                    }
                });
        ...
        setManager = (LineControllerView) findViewById(timechat.fcgz.sj.time.R.id.manager);
        ...
        final LineControllerView setQuiet = (LineControllerView) findViewById(timechat.fcgz.sj.time.R.id.setQuiet);
        setQuiet.setVisibility(canManage()&&!groupType.equals(GroupInfo.privateGroup) ? View.VISIBLE : View.GONE);
        if (canManage()){
            if (isQuiet()){
                setQuiet.setContent(getString(timechat.fcgz.sj.time.R.string.group_member_quiet_ing));
            }
                        ...
                        public void onClick(DialogInterface dialog, final int which) {
                            TIMGroupManager.getInstance().modifyGroupMemberInfoSetSilence(groupIdentify, userIdentify, getQuietTime(which),
                                    new TIMCallBack() {
                                        @Override
                                        public void onError(int i, String s) {
                                            Toast.makeText(GroupMemberProfileActivity.this, getString(timechat.fcgz.sj.time.R.string.group_member_quiet_err), Toast.LENGTH_SHORT).show();
                                        }
        ...
        LineControllerView nameCard = (LineControllerView) findViewById(timechat.fcgz.sj.time.R.id.groupCard);
        ...
        if (UserInfo.getInstance().getId().equals(userIdentify)){
            ...
            nameCard.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    EditActivity.navToEdit(GroupMemberProfileActivity.this, getResources().getString(timechat.fcgz.sj.time.R.string.group_member_change_card), userCard, CARD_REQ, new EditActivity.EditInterface() {
                        @Override
            ...
            TIMGroupManager.getInstance().modifyGroupMemberInfoSetRole(groupIdentify, userIdentify,
                    ...
                    new TIMCallBack() {
                        ...
                        public void onError(int i, String s) {
                            switch (i){
                                case 10004:
                                    Toast.makeText(GroupMemberProfileActivity.this, getString(timechat.fcgz.sj.time.R.string.group_member_manage_set_type_err), Toast.LENGTH_SHORT).show();
                                    break;
                                default:
                                    Toast.makeText(GroupMemberProfileActivity.this, getString(timechat.fcgz.sj.time.R.string.group_member_manage_set_err), Toast.LENGTH_SHORT).show();
                                    break;
                            }
                            //防止循环调用
                        ...
                        @Override
                        public void onSuccess() {
                            Toast.makeText(GroupMemberProfileActivity.this, getString(timechat.fcgz.sj.time.R.string.group_member_manage_set_succ), Toast.LENGTH_SHORT).show();
